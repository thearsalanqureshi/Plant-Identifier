// -------- Superior version - Just need to sizes and opacity of images
...
class FeatureCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Fixed: Proper responsive dimensions based on design
    final cardHeight = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveHeight(120, context) : ResponsiveHelper.responsiveHeight(100, context))
        : (isTablet ? ResponsiveHelper.responsiveHeight(165, context) : ResponsiveHelper.responsiveHeight(165, context));

    final iconSize = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveWidth(32, context) : ResponsiveHelper.responsiveWidth(28, context))
        : (isTablet ? ResponsiveHelper.responsiveWidth(36, context) : ResponsiveHelper.responsiveWidth(36, context));

    final bgSize = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveWidth(160, context) : ResponsiveHelper.responsiveWidth(140, context))
        : (isTablet ? ResponsiveHelper.responsiveWidth(150, context) : ResponsiveHelper.responsiveWidth(150, context));

    final titleFontSize = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveFontSize(20, context) : ResponsiveHelper.responsiveFontSize(18, context))
        : (isTablet ? ResponsiveHelper.responsiveFontSize(18, context) : ResponsiveHelper.responsiveFontSize(16, context));

    final subtitleFontSize = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveFontSize(14, context) : ResponsiveHelper.responsiveFontSize(13, context))
        : (isTablet ? ResponsiveHelper.responsiveFontSize(13, context) : ResponsiveHelper.responsiveFontSize(12, context));

    final topPadding = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveHeight(16, context) : ResponsiveHelper.responsiveHeight(14, context))
        : (isTablet ? ResponsiveHelper.responsiveHeight(16, context) : ResponsiveHelper.responsiveHeight(14, context));

    final leftPadding = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveWidth(20, context) : ResponsiveHelper.responsiveWidth(16, context))
        : (isTablet ? ResponsiveHelper.responsiveWidth(14, context) : ResponsiveHelper.responsiveWidth(12, context));

    final iconSpacing = isLarge
        ? (isTablet ? ResponsiveHelper.responsiveHeight(12, context) : ResponsiveHelper.responsiveHeight(8, context))
        : (isTablet ? ResponsiveHelper.responsiveHeight(48, context) : ResponsiveHelper.responsiveHeight(36, context));
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: Stack(
              ...
              children: [
                // Fixed: Proper background image positioning matching model UI
                if (config.backgroundImage != null)
                  Positioned(
                    bottom: isLarge ? -20 : -12,
                    right: isRtl ? null : (isLarge ? -8 : -4),
                    left: isRtl ? (isLarge ? -8 : -4) : null,
                    child: Opacity(
                      opacity: 0.9,
                      child: Image.asset(
                        config.backgroundImage!,
                ...
                // Fixed: Content positioning matching model UI
                Padding(
                  padding: EdgeInsets.only(
                    left: isRtl ? 0 : leftPadding,
                    right: isRtl ? leftPadding : 0,
                    top: topPadding,
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
...
class FeatureCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Responsive dimensions using your existing helper
    final cardHeight = _getCardHeight(isLarge, context);
    final iconSize = _getIconSize(isLarge, context);
    final bgSize = _getBackgroundSize(isLarge, context);
    final titleFontSize = _getTitleFontSize(isLarge, context);
    final subtitleFontSize = _getSubtitleFontSize(isLarge, context);
    final topPadding = _getTopPadding(isLarge, context);
    final horizontalPadding = _getHorizontalPadding(isLarge, context);
    final iconSpacing = _getIconSpacing(isLarge, context);
          ...
          child: Container(
            ...
            child: Stack(
              ...
              children: [
                ...
                if (config.backgroundImage != null)
                  Positioned(
                    bottom: isLarge ? context.h(-20) : context.h(-14),
                    right: isRtl ? null : (isLarge ? context.w(-8) : context.w(-4)),
                    left: isRtl ? (isLarge ? context.w(-8) : context.w(-4)) : null,
                    child: Transform(
                      alignment: Alignment.center,
                      transform: Matrix4.identity()..scale(isRtl ? -1.0 : 1.0, 1.0),
                      child: Opacity(
                ...
                Padding(
                  padding: EdgeInsets.fromLTRB(
                    isRtl ? 0 : horizontalPadding,
                    isRtl ? horizontalPadding : 0,
                    isRtl ? horizontalPadding : 0,
                    topPadding,
                  ),
                  child: Column(
  ...
  double _getCardHeight(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.h(120);
      } else if (context.isMobile) {
        return context.h(100);
      } else {
        return context.h(140);
      }
    } else {
      if (context.isTablet) {
        return context.h(180);
      } else if (context.isMobile) {
        return context.h(165);
      } else {
        return context.h(200);
      }
    }
  }

  double _getIconSize(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.w(32);
      } else if (context.isMobile) {
  ...
  double _getBackgroundSize(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.w(160);
      } else if (context.isMobile) {
        return context.w(140);
      } else {
        return context.w(180);
      }
    } else {
      if (context.isTablet) {
        return context.w(170);
      } else if (context.isMobile) {
        return context.w(150);
      } else {
        return context.w(190);
      }
    }
  }

  double _getTitleFontSize(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.sp(20);
      } else if (context.isMobile) {
        return context.sp(18);
      } else {
        return context.sp(22);
      }
    } else {
      if (context.isTablet) {
        return context.sp(18);
      } else if (context.isMobile) {
        return context.sp(16);
      } else {
        return context.sp(20);
      }
    }
  ...
  double _getSubtitleFontSize(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.sp(14);
      } else if (context.isMobile) {
        return context.sp(13);
      } else {
        return context.sp(15);
      }
    } else {
      if (context.isTablet) {
        return context.sp(13);
      } else if (context.isMobile) {
        return context.sp(12);
      } else {
        return context.sp(14);
      }
    }
  ...
  double _getTopPadding(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.h(16);
      } else if (context.isMobile) {
  ...
  double _getHorizontalPadding(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.w(18);
      } else if (context.isMobile) {
  ...
  double _getIconSpacing(bool isLarge, BuildContext context) {
    if (isLarge) {
      if (context.isTablet) {
        return context.h(10);
      } else if (context.isMobile) {
...
// Not Responsive 01/04/26 05:55am
/*import 'package:flutter/material.dart';
import 'package:flutter_svg/flutter_svg.dart';
import '../../../app/theme/app_colors.dart';
import '../../../app/theme/app_typography.dart';
